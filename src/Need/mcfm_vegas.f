...
************************************************************************
     ...
     & sigips(4),sdips(4),xcallwt,sigms,sdms
      character*4 mypart
      ...
      double precision lowint,virtint,realint,fragint,msbint
      double precision region(2*mxdim),lord_bypart(-1:1,-1:1)
      logical first,myreadin
      common/bypart/lord_bypart
      external lowint,virtint,realint,fragint,msbint
      data first/.true./
...
cmd Compute derivative term for msbar
      if ( ((mypart .eq. 'msbr') .or. (mypart .eq. 'tota')) 
     .  .and. (msbar) ) then
         part='msbr'
         readin=.false.
         writeout=.false.
         ncall=myncall*10
         call boundregion(ndim,region)
         call vegasnr(region,ndim,msbint,myinit,ncall,myitmx,
     .        0,sigms,sdms,chi)
         scale=myscale
         facscale=myfacscale
      endif
...
c--- calculate integration variables to be returned
      xinteg=sig+sigr+sigms+sigdk+sigWdk+sigfrag
      xerr=dsqrt(sd**2+sdr**2+sdms**2+sddk**2+sdWdk**2+sdfrag**2)      
